  ...
  describe('serialize', () => {
    it('BigInt를 string으로 변환한다', (done) => {
      ...
      interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe({
        next: (result) => {
          // THEN
          expect(result).toEqual({ value: bigIntValue.toString(10) });
          done();
        },
      });
    });
      ...
      interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe({
        next: (result) => {
          ...
          expect(result).toEqual({
            tx: {
              blockNumber: testValue.tx.blockNumber.toString(10),
    ...
    it('배열 내 BigInt를 변환한다', (done) => {
      ...
      interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe({
        next: (result) => {
          // THEN
          expect(result).toEqual({
            values: testValue.values.map((value) => value.toString(10)),
          });
          done();
        },
      });
      ...
      interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe({
        next: (result) => {
          ...
          expect(result).toEqual({
            ...
            logs: testValue.logs.map((elem) => {
              return { index: elem.index, value: elem.value.toString(10) };
            }),
            meta: {

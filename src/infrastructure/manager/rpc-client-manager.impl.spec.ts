...
import {
  RpcClientManagerImpl,
} from './rpc-client-manager.impl';
  ...
  describe('findChainByRoundRobin', () => {
    it('2개 이상의 PublcClient가 있을때, 라운드 로빈으로 가져오는지 확인한다. (http 2개)', (doneCallback) => {
      ...
      const ethPublicClients = createMockChainPublicClients(
        makePublicClients(mainnet, http, [ethFirstHttpUrl, ethSecondHttpUrl]),
        [],
      );
      ...
      // WHEN
      const firstPublicClient = manager.findChainByRoundRobin(targetChain);
      // THEN
      ...
      // WHEN
      const secondPublicClient = manager.findChainByRoundRobin(targetChain);
      // THEN
      ...
      // WHEN
      const thirdPublicClient = manager.findChainByRoundRobin(targetChain);
      // THEN
